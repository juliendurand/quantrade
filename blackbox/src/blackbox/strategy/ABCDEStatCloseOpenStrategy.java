...
public class ABCDEStatCloseOpenStrategy extends AInterdayStrategy {
	...
	public ABCDEStatCloseOpenStrategy(InterdayExchange exchange){
		_exchange = exchange;
	}
	...
	private void getDayTopGainer(){
		...
		for(String ticker : tickers){
			try{
				if(getExchange().getDailyCandle(ticker, 0).getVol20() >0.018){
					continue;
				}
